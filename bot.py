...
async def on_ready():
    await bot.change_presence(
        activity=discord.Game(
            name=f"pp help | pp new | grow your pp today~! ({bot.shard_count} shards, in {len(bot.guilds)} servers, with {len(await ud.fetch('user_id','userdata.pp'))} current users)"
        )
    )
    print("Bot ready!")
...
async def invite(ctx):
    await ctx.send(
        embed=discord.Embed(
            description="**[Invite the pp bot](https://discord.com/oauth2/authorize?client_id=735147633076863027&scope=bot&permissions=104713793)**"
        )
    )
...
async def git(ctx):
    await ctx.send(
        embed=discord.Embed(
            description="**[pp bot github](https://github.com/schlopp/ppbot)**"
        )
    )
...
async def support(ctx):
    await ctx.send("https://discord.gg/W8y2HfVK6m")
