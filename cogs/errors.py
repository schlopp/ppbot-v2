...
class CommandErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.DisabledCommand):
            ...
            embed.description = (
                f"Make sure you use numbers and words in the correct places"
            )
            embed.add_field(
                name="example:",
                value="`pp buy 10 fishing rod` ✅\n`pp show @obama` ✅\n`pp buy AAAAAAAAA fishing rod` ❌\n`pp show WAEDFSEWFSFEAWFEAW` ❌",
            )
            ...
            time_left = (
                f"{round(seconds, 1)} second"
                if round(seconds, 1) == 1.0
                else f"{round(seconds, 1)} seconds"
            )

            if minutes:
                time_left = (
                    f"{minutes} minute and {time_left}"
                    if minutes == 1
                    else f"{minutes} minutes and {time_left}"
                )
                for key, value in {"hours": hours, "days": days}.items():
                    if value:
                        time_left = (
                            f"{value} {key[:-1]}, {time_left}"
                            if value == 1
                            else f"{value} {key}, {time_left}"
                        )
            ...
            try:
                await ctx.send(
                    f"**Oopsie {ctx.author.display_name},** The command you're trying to use requires the permission to send links. Ask an admin to change pp bot's permissions."
                )
            except:
                pass
            finally:
