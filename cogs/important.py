...
class important(commands.Cog):
    ...
    async def show(self, ctx, user: discord.Member = None):
        ...
        if user.id == 339170260064141322:
            embed.add_field(
                name="MED!?",
                value="WOAH. I JUST REALISED THE USER YOU'RE TALKING ABOUT IS ACTUALLY MED. THIS IS SO COOL I LOVE MED AND ALL HER SUPPORT FOR ME.",
            )
        elif user.id == 351762383351447553:
            embed.add_field(
                name="SCAMMER RANK",
                value="Honestly, I fucking hate you noah. How the fuck did you get three blackjacks in a row AND double king? I'm crying right now and it's your fault. Thanks.",
            )
        ...
        if pp.multiplier["voted"]:
            embed.add_field(
                name="Stats",
                value=f"{pp.size} inches\n~~{pp.default_multiplier}x multiplier~~ **[__QUADRUPOLE__ VOTER REWARD BONUS! {pp.multiplier['multiplier']}x MULTIPLIER](https://top.gg/bot/735147633076863027/vote)**",
            )
        else:
            embed.add_field(
                name="Stats",
                value=f"{pp.size} inches\n{pp.multiplier['multiplier']}x multiplier **[You're currently missing out on a __QUADRUPOLE__ VOTER REWARD BONUS! Vote now to get a {pp.multiplier['multiplier']*4}x multiplier!](https://top.gg/bot/735147633076863027/vote)**",
            )
        invlist = []
    ...
    async def rename(self, ctx):
        ...
        try:
            ...
            if x:
                ...
                await pp.update()
                embed.description = (
                    f"{ctx.author.mention}, your {ppname}'s name is now **{newname}**"
                )
                return await ctx.send(embed=embed)
        except asyncio.TimeoutError:
            return await ctx.send(
                f"{ctx.author.mention}, time's up for changing name! Type faster next time"
            )
