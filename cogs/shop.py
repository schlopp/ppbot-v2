...
class shop(commands.Cog):
    ...
    async def buy(self, ctx, amount, *, item: str):
        ...
        if item not in shop.keys():
            raise ud.ShopItemNotFound(
                f'{ctx.author.mention}, **`"{item}"`** is not for sale at the moment. Check out the shop to see what\'s currently available'
            )
        ...
        if amount < 1:
            embed.description = (
                f"{ctx.author.mention}, you can't buy less than 1 of an item?????"
            )
            return await ctx.send(embed=embed)

        if pp.size < amount * shop[item]["price"]:
            raise ud.AmountNotEnough(
                f"{ctx.author.mention}, your pp isnt big enough! You need **{shop[item]['price'] * amount - pp.size} more inches** to buy this item! Type `pp grow` to grow your pp."
            )
