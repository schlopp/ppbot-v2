...
class Nerd(commands.Cog):
    ...
    async def db_shop_add(self, ctx, *, item_name: str):
        ...
        try:
            ...
            await ud.Shop.add_item(
                item_name, item_type, item_desc, default_price, multiplierDependant
            )
    ...
    async def db_shop_set(self, ctx, column, arg, *, item_name):
        await ud.runsql(
            "execute",
            f"UPDATE userdata.shopItems set {column} = {arg} WHERE item_name = '{item_name}'",
        )
    ...
    async def db_shop_delete(self, ctx, *, item_name: str):
        await ud.runsql(
            "execute", f"DELETE FROM userdata.shopItems WHERE item_name = '{item_name}'"
        )
    ...
    async def runsql(self, ctx, *, sql: str):
        ...
        if len(fetched) + 15 > 1500:
            ...
            return await ctx.send(
                ...
                file=discord.File(txtfile, "fetched.json"),
            )
        return await ctx.send(
            f"Executed in **{round(time_passed.total_seconds(), 3)}** seconds.\n```json\n{fetched}```"
        )
    ...
    async def messageinfo(self, ctx, message: typing.Optional[discord.Message]):
        ...
        headers = {"Authorization": f"Bot {self.config['admin']['TOKEN']}"}
        start_time = datetime.now()

        async with aiohttp.ClientSession() as session:
            if ctx.message.reference is None:
                payload = await session.get(
                    f"https://discord.com/api/channels/{ctx.channel.id}/messages/{message.id}",
                    headers=headers,
                )
            else:
                payload = await session.get(
                    f"https://discord.com/api/channels/{ctx.channel.id}/messages/{ctx.message.reference.message_id}",
                    headers=headers,
                )
        ...
        if len(payload) + 15 > 1500:
            ...
            return await ctx.send(
                f"Executed in **{round(time_passed.total_seconds(), 3)}** seconds.",
                file=discord.File(txtfile, "payload.json"),
            )
        return await ctx.send(
            f"Executed in **{round(time_passed.total_seconds(), 3)}** seconds.\n```json\n{payload}```"
        )
