...
from discord.types.interactions import InteractionData  # type: ignore
        ...
        with utils.UsingCommand(ctx):
            async with vbu.DatabaseConnection() as db:
                ...
                def action_check(action_interaction: discord.Interaction) -> bool:
                    if (
                        action_interaction.message is not None
                        and action_interaction.message.id != original_message.id
                    ...
                    if (
                        action_interaction.data is not None
                        and not utils.BlackjackAction.get(
                            typing.cast(
                                str, action_interaction.data.get("custom_id", "")
                            )
                        )
                    ):
