...
class FillInTheBlank:
    ...
    Attributes:
        ...
        prompts (`list` of `list` of `str`): The prompts for the user. Formatted with `{0}` as the obfuscated answer.
            E.g. `[["This is an {}", "EXAMPLE"]]` -> "`This is an _ _ _ _ _ _ _"
    """
    ...
    def get_random_prompt(self, hyperlink: str) -> typing.Tuple[str, str]:
        """
        Gets a random prompt and answer from the prompts list.
        """

        prompt, answer = random.choice(self.prompts)
        formatted_prompt = prompt.format(
            f"[`{' '.join(i if i == ' ' else '_' for i in answer)}`]({hyperlink})"
        )

        prefix = "" if prompt.startswith("[") else "`"
        suffix = "" if prompt.endswith(f"({hyperlink})") else "`"

        return answer, f"{prefix}{formatted_prompt}{suffix}"
