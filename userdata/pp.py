...
class Pp:
    ...
    async def fetch(
        ...
        fetched = await conn.fetch(
            """SELECT * FROM userdata.pp WHERE user_id=$1""", user_id
        )
        await conn.close()
                ...
                async with session.get(
                    ...
                    try:
                        ...
                        if r.status == 200 and data.get("voted", False):
                            self.multiplier = {
                                "multiplier": fetched[0]["multiplier"] * 4,
                                "voted": True,
                            }
                    except Exception:
                        pass
    ...
    async def update(self):
        conn = await asyncpg.connect(config["admin"]["PSQL"])
        await conn.execute(
            """UPDATE userdata.pp SET pp_name = $1, pp_size = $2, multiplier = $3 WHERE user_id = $4""",
            self.name,
            self.size,
